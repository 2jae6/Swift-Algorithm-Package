...
example(of: "creating and linking nodes") { // 유틸함수로 테스트 한다고 했쥬?
    let node1 = Node(value: 1) // Node 클래스 인스턴스 1
    let node2 = Node(value: 2) // Node 클래스 인스턴스 2
    let node3 = Node(value: 3) // Node 클래스 인스턴스 3 , 3개의 서로 다른 Node클래스의 인스턴스 임을 유의.
    
    node1.next = node2 // next 를 의미하는 프로퍼티에 node2 참조정보 할당.
    node2.next = node3
    
    print(node1) // extension 으로 구현한 Node 클래스의 description 출력되겠죠?
